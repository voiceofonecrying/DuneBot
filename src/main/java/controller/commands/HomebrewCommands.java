...
//              new SubcommandData("set-proxy-homeworld", "Set a different homeworld image if desired.").addOptions(atreidesKaramad)
    ...
    private static void addToken(DiscordGame discordGame, Game game) throws ChannelNotFoundException, InvalidGameStateException {
        String homebrewFactionName = discordGame.required(homebrewFaction).getAsString();
        String territoryName = discordGame.required(territory).getAsString();
        String homebrewToken = discordGame.required(homebrewTokenName).getAsString();
        ((HomebrewFaction) game.getFaction(homebrewFactionName)).addToken(game.getTerritory(territoryName), homebrewToken);
        discordGame.pushGame();
    }

    private static void removeToken(DiscordGame discordGame, Game game) throws ChannelNotFoundException, InvalidGameStateException {
        String homebrewFactionName = discordGame.required(homebrewFaction).getAsString();
        String territoryName = discordGame.required(territory).getAsString();
        String homebrewToken = discordGame.required(homebrewTokenName).getAsString();
        ((HomebrewFaction) game.getFaction(homebrewFactionName)).removeToken(game.getTerritory(territoryName), homebrewToken);
        discordGame.pushGame();
    }
}
